        ...
        <div className={style.BestSeller}>
            ...
            {initialFetch.currentFetch === true ? <Loader/>:<div className={style.products}>
                {bestSellers.map((item, index) =>{
                    return <Link onClick={()=> productDetailHandler(item.title,item.price,item.img)} style={{textDecoration:'none'}} key = {index} to="/productDetail"><Item  id = {item.id} img = {item.img} title = {item.title} price = {item.price}/></Link> 
                })}
            </div>}
        </div>
    )
